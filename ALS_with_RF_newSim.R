...
generateData <- function(n = 5000, NumCov=5,NumBi=2,NumNuiCov=75,sigma = 1, seed=seed) {
  ...
  set.seed(2398);
  x <- matrix(rep(0), nrow = n, ncol = NumCov )
  x[, 1:NumCov] <- matrix(rnorm(n*NumCov, 0, 1), ncol = NumCov)
  ...
  Y <-  0.1*trt + 0.406*x[,1]   - 0.239*x[,2]   + 0.703*x[,3]   - 0.090*x[,4]   - 0.299*x[,5] + 0.19*trt*x[,6]  + rnorm(n)
...
simData <- generateData(n = n,NumCov,NumBi,NumNuiCov, sigma = 1, seed = seedList[1] )
...
hist(prmt.sd, ylim = c(0, 100), breaks = 50, main = "Confounders dictate the .975 mark \n by lm")
