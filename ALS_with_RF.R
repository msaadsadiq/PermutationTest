...
als <- read.csv("als.csv", header = TRUE)
...
generateData <- function(n = 5000, NumCov=5,NumBi=2,NumNuiCov=75,sigma = 1, seed=seed) {
  ...
  trt <-rep(c(1,0), each = n/2)   #  outiside seed, Randomly permute the trt assignment
  
  set.seed(seed)
  eps = rnorm(n/2)
  
  # the small difference in yc, yt apart from 0.5 is random noise representing differences due to chance
  Y <-  0.5*trt + 0.406*x[,1]   - 0.239*x[,2]   + 0.703*x[,3]   - 0.090*x[,4]   - 0.299*x[,5]   + rnorm(n)
...
seedList=round(runif(numReps+1,0,n))
simData <- generateData(n = n,NumCov=5,NumBi=2,NumNuiCov=NumNuiCov, sigma = .1, seed = seedList[1] )


y = simData$Y
cov = simData[,!names(simData) %in% c("Y", "trt")]
trt = simData$trt
...
rt <- rfsrc(yt ~., data = TRTmxi, ntree = 1000)
rc <- rfsrc(yc ~., data = CNTLmxi, ntree = 1000)

pred_t_rf <- c( predict(rt, CNTLmxi)$predicted, rt$predicted.oob)
pred_c_rf <- c(rc$predicted.oob,  predict(rc, TRTmxi)$predicted)


pite.ind.rf <- pred_t_rf - pred_c_rf
pite.sd.rf <- sd(pite.ind.rf); pite.sd.rf
...
# permute nperm times
nperm = 1000
prmt <- prmtrf <- matrix(NA, nrow = n, ncol = nperm)
...
for(i in 1:nperm){
  ...
  ytp <- y[trt == 1]                                    # get the y of the treatment
  ycp <- y[trt == 0]                                    # get the y of the control 
  rt <- rfsrc(ytp ~ ., data.frame(xtp,ytp), ntree=1000 )                                   # regress the observed y on the shuffled x of the treatment group 
  rc <- rfsrc(ycp ~ ., data.frame(xcp,ycp), ntree=1000 )                                   # regress the observed y on the suffled x of the control group 

  # calculate PITE for nperm times
  pred_t_rf <- c( predict(rt, CNTLmxi)$predicted, rt$predicted.oob)
  pred_c_rf <- c(rc$predicted.oob,  predict(rc, TRTmxi)$predicted)

  prmtrf[,i] <- pred_t_rf - pred_c_rf                                                            # calculate PITE for the ith permutation replicate
}


# calculate the standard deviatoin of PITEs for each permutation
prmt.sd <- apply(prmt, 2, sd, na.rm = T) 
prmt.sd.rf <- apply(prmtrf, 2, sd, na.rm = T) 
...
hist(prmt.sd.rf, xlim = c(0, 1.1), ylim = c(0, 80), breaks = 50, main = "Histogram of prmt.sd \n No interaction")
abline(v = pite.sd.rf, col = 'red')


text(pite.sd.rf, 80, paste("pite.sd = ", round(pite.sd, 3)), col = 'blue')
